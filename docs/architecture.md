...
启动时 UI 先查询现有 SQLite 快照并立即可交互；索引器随后在后台执行增量同步。`usage-sync-completed` 会失效 `dashboard`、`workspaces`、`sessions`、`session-detail`、`usage-events`、`models`、`heatmap`、`tools` 和 `diagnostics` Query key；失败事件只刷新同步状态。首次导入同样不阻塞 UI，并提供真实文件/字节进度、速度、取消、断点续传和错误恢复。Bootstrap 同时返回 `platform`、实际分析目录与数据库路径，前端不推断 Windows/macOS 文件系统布局。
